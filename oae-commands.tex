...
%  Macro that creates a figure from the global FIGURES dir (margin)
\usepackage{ifthen}

\NewDocumentCommand{\marginfig}{O{width=\linewidth} m o o o o}{%
  \begin{marginfigure}\IfValueT{#6}{[#6]}%
    \IfValueTF{#5}{#5}{\centering}%
    \includegraphics[#1]{#2}%  % ← Just use the filename, let graphicspath handle it
    \IfValueTF{#3}{%
      \caption{#3}%
    }{%
      \caption{#2}%
...
\NewDocumentCommand{\fig}{O{width=\linewidth} m o o}{%
  \begin{figure}
    \centering
    \includegraphics[#1]{#2}% 
    \IfValueTF{#3}{%
...
\DeclareMathOperator*{\convms}{\smash{\overset{L_2}{\rightarrow}}}
...
\let\originalbibliographystyle\bibliographystyle

% Redefine to suppress bibliographies in chapters when compiling main document


%%%% DAE
\newcommand{\CELL}{\texttt{CELL}\xspace}
\newcommand{\LINK}{\texttt{LINK}\xspace}
\newcommand{\IUI}{\texttt{IUI}\xspace}
\newcommand{\CELLs}{\CELL{}s\xspace}        % plural
\newcommand{\LINKs}{\LINK{}s\xspace}        % plural
\newcommand{\IKTYKTIKT}{\texttt{TIK\hspace{1pt}TYK\hspace{1pt}TIK} \xspace}
\newcommand{\TTT}{\texttt{TIK\hspace{1pt}TYK\hspace{1pt}TIK} \xspace}
